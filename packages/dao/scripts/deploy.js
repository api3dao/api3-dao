...
module.exports = async callback => {
    try {
        ...
        const secondaryAgent = getEventArgument(tx, 'Api3DaoDeployed', 'secondaryAgent');
        ...
        const set_tx = await api3Pool.setDaoApps(mainAgent, secondaryAgent, mainVoting, secondaryVoting);
        console.log("Voting Primary Agent: " + (getEventArgument(set_tx, 'SetDaoApps', 'votingAppPrimary').toString()));
        console.log("Voting Secondary Agent: " + (getEventArgument(set_tx, 'SetDaoApps', 'votingAppSecondary').toString()));
        console.log("Dao Primary Agent: " + (getEventArgument(set_tx, 'SetDaoApps', 'agentAppPrimary').toString()));
        console.log("Dao Secondary Agent: " + (getEventArgument(set_tx, 'SetDaoApps', 'agentAppSecondary').toString()));
    } catch (error) {
        callback(error);
    }
    callback();
