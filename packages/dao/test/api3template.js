...
contract('Api3Template', ([_, deployer, tokenAddress, authorized]) => { // eslint-disable-line no-unused-vars
  ...
  it('installs the requested application correctly', async () => {
    ...
    await assertRole(acl, dao, { address: votingPrimary.address }, 'APP_MANAGER_ROLE');
    await assertRole(acl, acl, { address: votingPrimary.address }, 'CREATE_PERMISSIONS_ROLE');

    assert.isTrue(await votingPrimary.hasInitialized(), 'voting not initialized');
    assert.equal((await votingPrimary.voteTime()).toString(), VOTING_DURATION_1);
    assert.equal((await votingPrimary.supportRequiredPct()).toString(), SUPPORT_1);
    assert.equal((await votingPrimary.minAcceptQuorumPct()).toString(), ACCEPTANCE_1);
    ...
    assert.isTrue(await agentPrimary.hasInitialized(), 'Main agent not initialized');
    ...
    await assertRole(acl, votingPrimary, { address: votingPrimary.address }, 'MODIFY_SUPPORT_ROLE', {address: agentPrimary.address});
    await assertRole(acl, votingPrimary, { address: votingPrimary.address }, 'MODIFY_QUORUM_ROLE', {address: agentPrimary.address});
    await assertRole(acl, votingSecondary, { address: votingPrimary.address }, 'MODIFY_SUPPORT_ROLE', {address: agentPrimary.address});
    await assertRole(acl, votingSecondary, { address: votingPrimary.address }, 'MODIFY_QUORUM_ROLE', {address: agentPrimary.address});

    await assertRole(acl, votingPrimary, { address: votingPrimary.address }, 'CREATE_VOTES_ROLE', { address: authorized });
    await assertRole(acl, votingSecondary, { address: votingPrimary.address }, 'CREATE_VOTES_ROLE', { address: authorized });

    // Voting can execute all actions on agent which is connected to this voting
    await assertRole(acl, agentPrimary, { address: votingPrimary.address }, 'EXECUTE_ROLE', { address: votingPrimary.address });
    await assertRole(acl, agentPrimary, { address: votingPrimary.address }, 'RUN_SCRIPT_ROLE', { address: votingPrimary.address });


    await assertRole(acl, agentSecondary, { address: votingPrimary.address }, 'EXECUTE_ROLE', { address: votingSecondary.address });
    await assertRole(acl, agentSecondary, { address: votingPrimary.address }, 'RUN_SCRIPT_ROLE', { address: votingSecondary.address });

    await api3Pool.setDaoApps(agentPrimary.address, agentSecondary.address, votingPrimary.address, votingSecondary.address);
  })
