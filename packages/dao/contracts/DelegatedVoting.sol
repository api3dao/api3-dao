...
contract Voting is IForwarder, AragonApp {
    ...
    */
    ...
    function getVotingPower(address _voter, uint256 _voteId)
    external returns(uint256) {
        uint256 atBlock = votes[_voteId]
        ...
        if (delegates[_voter]) {
            power = power.add(getDelegatedVotingPower(_voter, _voteId, _atBlock));
        }
        return power;
    }

    function getDelegatedVotingPower(address _voter, uint256 _voteId, uint256 _atBlock) 
    ...
    */
    function _newVote(bytes _executionScript, string _metadata, bool _castVote, bool _executesIfDecided) internal returns (uint256 voteId) {
        ...
        require(lastUserProposalTimes[msg.sender] < block.now.sub(604800));
        uint256 proposerVotingPower = getVotingPower(msg.sender);
        require(proposerVotingPower >= minimumVotingPowerToPropose);
    ...
    */
    function _vote(uint256 _voteId, bool _supports, address _voter, bool _executesIfDecided) internal {
        ...
        uint256 voterStake = getVotingPower(_voter, _voteId);
