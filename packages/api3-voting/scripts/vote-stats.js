...
const appAddress =
  process.env.APP_ADDRESS || "0xcfee4d3078f74197ce77120dbfe6d35f443cab1c";
const voteId = process.env.VOTE_ID || "0";

const tenPow = (x) => web3.toBigNumber(10).pow(x);
const formatNumber = (number, decimals) =>
  number.div(tenPow(decimals)).toString();
...
module.exports = async (cb) => {
  ...
  voting.CastVote({ voteId }, { fromBlock }).get(async (err, votes) => {
    ...
    const proccessedVotes = await Promise.all(
      votes.map(async ({ transactionHash, args }) => {
