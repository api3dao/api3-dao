...
//       // check unstaked
//       const user = await pool.users(accounts[1]);
//       const unstaked = await user.unstaked;
...
//         const user = await pool.users(accounts[1]);
//         const unstakeAmount = await user.unstakeAmount;
//         expect(unstakeAmount).to.equal(testValue);
//         // check unstake request timestamp
//         const now = await getBlockTimestamp();
//         const unstakeScheduleAt = await user.unstakeScheduledFor;
...
//     const staker = pool.connect(hre.waffle.provider.getSigner(1));
//     const user = await pool.users(accounts[1]);
//     const alreadyUnstaked = await user.unstaked;
...
//       const user = await pool.users(accounts[1]);
//       const expectedUnstaked = await user.unstaked;
...
export async function resetUnstakeRequest(staker: TestPool, address: string, amount: BigNumber) {
  ...
  const alreadyUnstaked = user.unstaked;
  ...
  return [await user.unstakeScheduledFor, await user.unstakeAmount];
}
...
export const resetUnstakedTo = async (amount: BigNumber, stakerAddress: string, ownerAddress: string, token: Api3Token, pool: TestPool) => {
  ...
  const user = await staker.users(stakerAddress);
  const minWait = await pool.rewardEpochLength();
