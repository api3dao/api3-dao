...
import "./interfaces/IApi3Voting.sol";
...
contract Convenience is Ownable {
    ...
    mapping(VotingAppType => mapping(uint256 => string))
        public votingAppTypeToVoteIdToDiscussionUrl;
    ...
    constructor(address api3PoolAddress) {
        ...
        api3Token = IERC20Metadata(
            address(IApi3Pool(api3PoolAddress).api3Token())
        );
    }
    ...
    function setDiscussionUrl(
        ...
        votingAppTypeToVoteIdToDiscussionUrl[votingAppType][
            voteId
        ] = discussionUrl;
        ...
        )
    {
        ...
        (
            ...
            userUnstakeAmount,
            userUnstakeShares,
            userUnstakeScheduledFor, // lastDelegationUpdateTimestamp
        ...
        )
    {
        ...
        for (uint256 i = 0; i < erc20Addresses.length; i++) {
            ...
            balancesOfPrimaryAgent[i] = erc20.balanceOf(
                api3Pool.agentAppPrimary()
            );
            balancesOfSecondaryAgent[i] = erc20.balanceOf(
                api3Pool.agentAppSecondary()
            );
        }
        names[erc20Addresses.length] = "Ethereum";
        symbols[erc20Addresses.length] = "ETH";
        decimals[erc20Addresses.length] = 18;
        balancesOfPrimaryAgent[erc20Addresses.length] = address(
            api3Pool.agentAppPrimary()
        ).balance;
        balancesOfSecondaryAgent[erc20Addresses.length] = address(
            api3Pool.agentAppSecondary()
        ).balance;
        ...
        )
    {
        ...
        for (uint256 i = 0; i < voteIds.length; i++) {
            ...
            (
                ...
                snapshotBlock,
                supportRequired[i],
                minAcceptQuorum[i], // yea // nay
                ,
                ,
                votingPower[i],
                script[i]
            ) = api3Voting.getVote(voteIds[i]);
            userVotingPowerAt[i] = api3Pool.userVotingPowerAt(
                userAddress,
                snapshotBlock
            );
            discussionUrl[i] = votingAppTypeToVoteIdToDiscussionUrl[
                votingAppType
            ][voteIds[i]];
        }
    }
        ...
        )
    {
        ...
        for (uint256 i = 0; i < voteIds.length; i++) {
            ...
            (
                ,
                // open
                executed[i], // startDate
                ,
                snapshotBlock, // supportRequired // minAcceptQuorum
                ,
                ,
                yea[i],
                nay[i], // votingPower
            ...
            if (delegateAt[i] == address(0)) {
                voterState[i] = api3Voting.getVoterState(
                    voteIds[i],
                    userAddress
                );
            } else {
                delegateState[i] = api3Voting.getVoterState(
                    voteIds[i],
                    delegateAt[i]
                );
            }
        ...
        returns (uint256[] memory voteIds)
    {
        ...
        for (uint256 i = votesLength; i > 0; i--) {
            (
                bool open, // executed
                ,
                uint64 startDate, //snapshotBlock // supportRequired // minAcceptQuorum // yea // nay // votingPower // script
                ,
                ,
                ,
                ,
        ...
        for (uint256 i = votesLength; i > 0; i--) {
            ...
            (
                bool open, // executed // startDate // snapshotBlock // supportRequired // minAcceptQuorum // yea // nay // votingPower // script
                ,
                ,
                ,
                ,
